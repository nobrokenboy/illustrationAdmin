...
export default {
    ...
    computed:{
        ...
        username(){//获取用户名
            return this.$store.state.userMsg!=null?this.$store.state.userMsg.userName:"";
        },
    },
    metaInfo:Object.assign({title:"插画设计管理页面"},metaHeader),
    watch:{
        '$route'(){
            this.breadCrumbList=[];
            this.getBreadCrumb();
    ...
    methods:{
        ...
        logout(){//退出登录
            ask.user.logout({},res=>{
                console.log(res);
                if(res.code==0){
                    this.$store.commit('illustrationUser',null);
                    this.$router.push({
                        path:"/login",
                    });
                }else{
